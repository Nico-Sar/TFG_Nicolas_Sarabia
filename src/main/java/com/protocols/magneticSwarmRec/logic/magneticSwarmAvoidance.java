...
public class magneticSwarmAvoidance extends Thread {
    ...
    public magneticSwarmAvoidance(int numUAV) {
        ...
        this.dirRatio = magneticSwarmRecSimProperties.dirRatio;
    ...
    private RepulsionResult calculateRepulsionAndSlowdown() {
        ...
        for (Pair<Location3DUTM, Double> obs : communication.getObstaclesWithHeading()) {
            ...
            if (dist <= frdDir) {
                ...
                double directionFactor = Math.max(Math.cos(dirFactor * angle), dirRatio);
